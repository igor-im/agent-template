...
class UserInput(BaseModel):
    """Basic user input for the agent.py."""

    message: str = Field(
        description="User input to the agent.py.",
        examples=["What is the weather in Tokyo?"],
    )
    model: str = Field(
        description="LLM Model to use for the agent.py.",
        default="gpt-4",
        examples=["gpt-4"],
    )
...
class StreamInput(UserInput):
    """User input for streaming the agent.py's response."""
...
class AgentResponse(BaseModel):
    """Response from the agent.py when called via /invoke."""

    message: dict[str, Any] = Field(
        description="Final response from the agent.py, as a serialized LangChain message.",
        examples=[
            {
                "message": {
                    "type": "ai",
